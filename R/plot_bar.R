...
plot_bar <- function(data_frame)
{
  ...
  theme <- theme(axis.text.y=element_blank(), axis.ticks.y=element_blank(), axis.title.y=element_blank(), legend.position=('none'), axis.title.x=element_text(size=16), plot.margin=unit(c(1,-1,.5,.5), 'lines'))
  y_limits <- ylim(100, 0)
